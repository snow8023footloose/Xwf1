  ...
  <div class="cartcontrol">
    <transition name="move">
      <div class="cart-decrease icon-add_circle" v-show="food.count>0" @click.stop.prevent="decreaseCart">
        <i class="el-icon-remove-outline inner icon-remove_circle_outline" :style="{color:SColor}"></i>
      </div>
    </transition>
    <div class="cart-count" v-show="food.count>0">{{food.count}}</div>
    <i class="el-icon-circle-plus cart-add icon-add_circle" @click.stop.prevent="addCart" :style="{color:SColor}"></i>
  </div>
</template>
